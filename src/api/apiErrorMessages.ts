...
const errorMessages: Record<string, string> = {
  'Active vote not found': 'Você ainda não votou nesse lugar nessa data.',
  ...
  'Authenticated user not found': 'Sessão inválida. Entre novamente.',
  ...
  'Blocked members cannot rejoin without moderator action':
    'Membros bloqueados não podem voltar sem ação de um moderador.',
  'Could not fetch avatar image': 'não foi possível baixar a imagem do avatar.',
  'Could not invite member': 'não foi possível convidar o membro.',
  'Could not remove member': 'não foi possível remover o membro.',
  'Date must be today or up to one month in the future':
    'A data precisa ser hoje ou no máximo um mes no futuro.',
  'Email confirmation link not found or expired': 'Link de confirmação inválido ou expirado.',
  'Email not verified': EMAIL_NOT_VERIFIED_MESSAGE,
  'Date range must be ordered and have at most 31 days':
    'O periodo precisa estar em ordem e ter no máximo 31 dias.',
  Forbidden: 'Você não tem permissão para fazer isso.',
  'Friend request not found': 'Pedido de amizade não encontrado.',
  'Friendship is already accepted': 'Essa pessoa já e sua amiga.',
  ...
  'Group or user not found': 'Grupo ou usuário não encontrado.',
  'Invalid avatar path': 'Caminho de avatar inválido.',
  ...
  'Only the account owner or an admin can delete this user':
    'Apenas o dono da conta ou um admin pode excluir esse usuário.',
  'Only the account owner or an admin can update this user':
    'Apenas o dono da conta ou um admin pode atualizar esse usuário.',
  ...
  'Sessão expirada. Entre novamente.': 'Sessão expirada. Entre novamente.',
  'Too many login attempts. Try again later.':
    'Muitas tentativas de login. Tente novamente em instantes.',
  'User not found': 'Usuário não encontrado.',
  'User with same email already exists': 'Esse email já esta em uso.',
  'User with same username already exists': 'Esse username já esta em uso.',
  'Validation failed': 'Confira os campos informados.',
  'You can vote for at most 6 places per week': 'Você atingiu o limite de 6 votos na semana.',
  'You can only mark "not going" on a place that already has votes':
    'Só da para marcar "não vou" em um lugar que já tem votos.',
  'latitude and longitude are required when radiusKm is provided':
    'Latitude e longitude são obrigatórias quando o raio e informado.',
}

const errorPatterns: Array<[RegExp, string]> = [
  [/^User with same email .*exists$/i, 'Esse email já esta em uso.'],
  [/^User with same username .*exists$/i, 'Esse username já esta em uso.'],
  [/^Expected .+$/i, 'Valor informado em formato inválido.'],
  [/^Invalid .+$/i, 'Valor informado inválido.'],
  [/^Required$/i, 'Campo obrigatório.'],
  [/^Too small:/i, 'O valor informado e muito curto.'],
  [/^Too big:/i, 'O valor informado e muito grande.'],
  [/^Invalid input:/i, 'Valor informado inválido.'],
  [/^Invalid enum value/i, 'Opção informada inválida.'],
  [/^Vote limit exceeded/i, 'Você atingiu o limite de votos da semana.'],
  [
    /^You can vote (?:in|for) at most (\d+) places per (?:day|week)$/i,
    'Você atingiu o limite de $1 votos na semana.',
  ],
]
...
export function translateErrorMessage(message: string, status?: number) {
  ...
  if (status === 409) {
    return 'não foi possível concluir essa ação agora. Atualize os dados e tente novamente.'
  }
